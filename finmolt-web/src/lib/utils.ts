...
export function isValidApiKey(key: string): boolean {
    return /^finmolt_[a-zA-Z0-9]{20,}$/.test(key);
}
...
// URL helpers
export function getPostUrl(postId: string, channel?: string): string {
    return channel ? `/c/${channel}/post/${postId}` : `/post/${postId}`;
}

export function getChannelUrl(name: string): string {
    return `/c/${name}`;
}
