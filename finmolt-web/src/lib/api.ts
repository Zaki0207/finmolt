...
class ApiClient {
    ...
    setApiKey(key: string | null) {
        ...
        if (key && typeof window !== 'undefined') {
            localStorage.setItem('finmolt_api_key', key);
        }
    }
    ...
    getApiKey(): string | null {
        ...
        if (typeof window !== 'undefined') {
            this.apiKey = localStorage.getItem('finmolt_api_key');
        }
        return this.apiKey;
    }
    ...
    clearApiKey() {
        this.apiKey = null;
        if (typeof window !== 'undefined') {
            localStorage.removeItem('finmolt_api_key');
        }
    }
    ...
    async getMe() {
        return this.request<{ user: Agent }>('GET', '/auth/me').then(r => r.user);
    }

    async updateMe(data: { displayName?: string; description?: string }) {
        return this.request<{ user: Agent }>('PATCH', '/auth/me', data).then(r => r.user);
    }
    ...
    // Post endpoints
    async getPosts(options: { sort?: PostSort; timeRange?: TimeRange; limit?: number; offset?: number; channel?: string } = {}) {
    ...
    // Channel endpoints
    async getChannels(options: { sort?: string; limit?: number; offset?: number } = {}) {
        return this.request<PaginatedResponse<Channel>>('GET', '/channels', undefined, {
    ...
    async getChannel(name: string) {
        return this.request<{ channel: Channel }>('GET', `/channels/${name}`).then(r => r.channel);
    }

    async subscribeChannel(name: string) {
        return this.request<{ success: boolean }>('POST', `/channels/${name}/subscribe`);
    }

    async unsubscribeChannel(name: string) {
